...
<?php else: ?>
    <div class="event-item">
        <h4 class="a-blog-item-title">&#8594; <?php echo link_to( truncate_text($aEvent['title'],35), 'a_event_post', $aEvent) ?></h4>
        <span class="datelocation">
            <?php include_partial('aEvent/dateRangeCompact', array('aEvent' => $aEvent,)) ?> 
            <?php if($aEvent['location']): ?>
                (<?php echo truncate_text(aString::firstLine($aEvent['location']),20) ?>)
            <?php endif ?>
        </span>
    </div>
<?php endif ?>